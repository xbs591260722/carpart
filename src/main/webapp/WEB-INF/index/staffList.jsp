...
<script type="text/plain" id="html_smiitem">
    <div class="smiitem"{{if elementId}} id="${elementId}"{{else}} data-value="${value}"  data-target="${target}"{{/if}}>
        <div class="smiitxt">
          ${value}
        </div>
        <div class="smiic">
            <span class="yy-icon yy-sic"></span>
        </div>
    </div>
</script>
</head><!--[if lt IE 9]>
...
(function(){window.onload=function(){var imgs=document.getElementsByTagName("img");
for(var i=0;i<imgs.length;i++){var data_src=imgs[i].getAttribute("data-src");
                          ...
                          <c:forEach var="staff" items="${staffList}" varStatus="status">
                              <tr>
                                ...
                                <td width="60"><c:if test="${staff.userStatus==1}">在职</c:if><c:if test="${staff.userStatus==2}"><span style="color:red">离职</span></c:if></td>
                                 <td width="100"><fmt:formatDate value="${staff.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
